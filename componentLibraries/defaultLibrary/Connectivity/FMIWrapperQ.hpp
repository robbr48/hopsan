...
private:
    ...
    std::map<fmi2ValueReference, double*> mPortRealInputs;
    std::map<fmi2ValueReference, double*> mPortRealOutputs;
...
public:
            ...
            for(size_t i=0; i<ports.size(); ++i) {
                ...
                for(size_t j=0; j<specs.size()-1; ++j) {
                    ...
                    if(pNode->getDataDescription(j)->varType == TLMType) {
                        mPortRealInputs[vr] = getSafeNodeDataPtr(pPort, (int)j);
                    }
                    else {

                        mPortRealOutputs[vr] = getSafeNodeDataPtr(pPort, (int)j);
                    }
                }
            }
            ...
            for(size_t i=0; i<ports.size(); ++i) {
                ...
                for(size_t j=0; j<specs.size()-1; ++j) {
                    ...
                    if(pNode->getDataDescription(j)->varType == TLMType) {
                        ...
                        for(it = mFloat64Inputs.begin(); it != mFloat64Inputs.end(); it++) {
                            if(it->first == vr) {
    ...
    void simulateOneTimestep()
    {
        if(mFmiVersion == fmiVersion1) {
            ...
            for(const auto &input : mPortRealInputs) {
                fmi2_setReal(fmu, &input.first, 1, input.second);
            }
            ...
            for(const auto &output : mPortRealOutputs) {
                fmi2_getReal(fmu, &output.first, 1, output.second);
            }
        }
        else { //FMI 3
            if(NULL == fmu) {
